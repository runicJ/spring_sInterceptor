...
// 게시판을 가져오기 위한 인터셉트
public class BoardInterceptor extends HandlerInterceptorAdapter {  // 상속받으면 interceptor로서의 역할을 하게됨  // 부모가 추상클래스(일반x)=>부모도 implement함. 부모 위 인터페이스O => 부모 클래스 아래 메소드가 있지만 다 받아서 사용 안해도 됨
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if(level > 3) {  // 99번인 경우만 못들어감
			RequestDispatcher dispatcher = request.getRequestDispatcher("/message/loginNo");  // 빠꾸 forward처리
